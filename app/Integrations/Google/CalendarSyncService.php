...
/**
 * Pushes Bookora appointments to a staff member's Google Calendar and exposes
 * their external busy intervals to the availability engine.
 *
 * Availability reads a short-lived cache (warmed by sync/cron), never a live API
 * call, so slot computation stays fast and resilient.
 */
final class CalendarSyncService {

	private const BUSY_TTL = 600;
	private const HORIZON  = 21; // days to pull busy for.
	...
	/**
	 * Push (create or update) an appointment to Google.
	 *
	...
	public function push_for_appointment( int $appointment_id ): void {
		...
		if ( ! empty( $appt['external_event_id'] ) ) {
			$this->client->update_event( $token, $calendar, (string) $appt['external_event_id'], $event );
		...
		if ( ! is_wp_error( $event_id ) && '' !== $event_id ) {
			$this->appointments->update( $appointment_id, array( 'external_event_id' => $event_id ) );
		}
	}

	/**
	 * Delete an appointment's Google event.
	 *
	 * @param int $appointment_id Appointment id.
	 * @return void
	 */
	public function delete_for_appointment( int $appointment_id ): void {
		$appt = $this->appointments->find( $appointment_id );
		if ( null === $appt || empty( $appt['staff_id'] ) || empty( $appt['external_event_id'] ) ) {
			return;
		}
		...
		$this->client->delete_event( $token, $this->calendar_id( (int) $appt['staff_id'] ), (string) $appt['external_event_id'] );
		$this->appointments->update( $appointment_id, array( 'external_event_id' => null ) );
	}

	/**
	 * Pull a staff member's free/busy from Google and cache it.
	 *
	 * @param int $staff_id Staff id.
	 * @return void
	 */
	...
	private function cache_key( int $staff_id ): string {
		return 'bookora_gbusy_' . $staff_id;
	}
